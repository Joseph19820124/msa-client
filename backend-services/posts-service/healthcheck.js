// Health check script for Posts Service
...
const config = {
  ...
  mongoUri: process.env.MONGODB_URI || 'mongodb://localhost:27017/posts_db',
...
async function checkMongoDB() {
  ...
  try {
    ...
    // Check if we can read from posts collection
    const postsCollection = client.db().collection('posts');
    await postsCollection.findOne({}, { projection: { _id: 1 } });
